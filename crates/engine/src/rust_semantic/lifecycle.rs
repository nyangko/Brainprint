//! Wiring the Rust semantic tier to the Workspace lifecycle.
//!
//! ```text
//! filesystem change
//!   → classify: source, or the project's own definition?
//!   → affected owners, from persisted basis + structural dependents
//!   → withdraw their semantic contributions
//!   → structural replacement (I2/I3)
//!   → source:   Brainprint-owned document sync
//!     manifest: watched files + reloadWorkspace + the quiescent barrier
//!   → refresh affected owners
//!   → publish (task 3) → merge (task 4)
//! ```
//!
//! ## Where Rust's module tree lives, and why it changes the classes
//!
//! In C# a project's Compile set comes from MSBuild, so adding a `.cs`
//! is a project change even though no project file was edited. Rust is
//! the other way round: the module tree is written in *source*
//! (`mod runner;`), and Cargo's manifest says nothing about which files
//! are modules. So adding `new.rs` adds no module until some file says
//! `mod new;`, and that saying is a source edit like any other.
//!
//! The consequence is a smaller reload class than C#'s. Only the
//! project's own definition — a manifest, the lockfile, the Cargo
//! config, the toolchain file — needs [`reload_projects`]. Everything
//! else, including a module tree change, is [`synchronize_documents`].
//!
//! ## The barrier
//!
//! `experimental/serverStatus` with `quiescent: true`, and it exists
//! only because the handshake asked for it. Nothing here sleeps.

use std::{
    collections::{BTreeMap, BTreeSet},
    error::Error,
    fmt, fs,
    path::Path,
};

use brainprint_core::ResourceId;
use rusqlite::{Connection, OptionalExtension, params};

use super::{
    RefreshOutcome, RefreshRequest, RustSemanticError, adapter,
    protocol::{ProjectExecutionTrust, WatchedChange, WatchedChangeKind, method, path_to_uri},
};
use crate::{
    db, graph_lifecycle,
    merge::{self, MergeOutcome},
    resource::{Resource, ResourceLanguage},
    semantic::{AnalysisContext, CapabilityReport},
    semantic_index::{
        BACKEND_UNAVAILABLE_CODE, CONFIG_CHANGED_CODE, ConfigBasis, CurrentInputs, SemanticIndex,
        SemanticIndexError, SemanticOwner, SemanticStatus,
    },
};

/// The Level B vocabulary, shared with every other backend.
pub use crate::semantic_lifecycle::{BackendReadiness, SemanticAvailability, availability};

/// The languages this backend answers for.
pub const SERVED_LANGUAGES: [ResourceLanguage; 1] = [ResourceLanguage::Rust];

/// The file that defines a Cargo package or workspace.
pub const MANIFEST: &str = "Cargo.toml";

/// The file that pins the dependency graph.
pub const LOCKFILE: &str = "Cargo.lock";

/// Files that change what the compilation *is* without being a
/// manifest.
pub const ENVIRONMENT_NAMES: [&str; 3] = ["rust-toolchain.toml", "rust-toolchain", "config.toml"];

// ---------------------------------------------------------------------
// The Cargo world
// ---------------------------------------------------------------------

/// Which package owns which source file, and what governs them.
#[derive(Debug, Clone, Default, PartialEq, Eq)]
pub struct RustProjectConfig {
    /// Every `Cargo.toml` in the Workspace, by path key.
    pub manifests: Vec<Resource>,
    /// `Cargo.lock`, when it is committed.
    pub lockfile: Option<Resource>,
    /// `rust-toolchain.toml`, `.cargo/config.toml`.
    pub environment: Vec<Resource>,
    /// The nearest owning package for each active Rust Resource.
    ///
    /// A *package*, which is not a crate and not a module: one package
    /// can build a lib, a bin and several test targets, and a crate
    /// root is one file inside it. They are kept apart because their
    /// names usually match and their meanings never do.
    pub owning_package: BTreeMap<ResourceId, String>,
    /// The package name each manifest declares, by manifest path key.
    pub package_names: BTreeMap<String, String>,
    /// The feature selection Brainprint supplied to the backend.
    ///
    /// Recorded rather than inferred: a source file means different
    /// things under different features, and one context claims one
    /// selection.
    pub features: Vec<String>,
    /// Whether default features were disabled.
    pub no_default_features: bool,
    /// The target triple the analysis is for, when one was chosen.
    pub target: Option<String>,
    /// What the Workspace is allowed to load.
    pub trust: ProjectExecutionTrust,
}

impl RustProjectConfig {
    /// The task 3 [`ConfigBasis`] this configuration contributes.
    ///
    /// The trust mode, the safe-configuration switches and the feature
    /// selection are all in here, because each of them changes what the
    /// answers *mean* and not merely how fast they arrive.
    #[must_use]
    pub fn basis(&self) -> ConfigBasis {
        let mut basis = ConfigBasis::new()
            .with("project_trust", self.trust.as_str())
            .with("backend_configuration", super::protocol::SAFE_CONFIGURATION)
            .with("features", self.features.join(","))
            .with(
                "no_default_features",
                if self.no_default_features {
                    "true"
                } else {
                    "false"
                },
            )
            .with("target", self.target.clone().unwrap_or_default());
        for resource in self
            .manifests
            .iter()
            .chain(self.lockfile.iter())
            .chain(&self.environment)
        {
            basis = basis.with(
                format!("cargo:{}", resource.path_key),
                content_identity(resource),
            );
        }
        basis
    }

    /// Whether project code would run if a question needed it.
    #[must_use]
    pub const fn loads_projects(&self) -> bool {
        self.trust.may_load_projects()
    }

    /// The workspace manifest, when there is one.
    ///
    /// The one at the shallowest path: Cargo allows exactly one
    /// workspace root above its members.
    #[must_use]
    pub fn workspace_manifest(&self) -> Option<&Resource> {
        self.manifests
            .iter()
            .min_by_key(|resource| resource.path_key.matches('/').count())
    }
}

/// Find the Cargo files and package ownership the Workspace analyses
/// under.
///
/// Ownership is nearest-ancestor: the `Cargo.toml` in the closest
/// directory above a source file. That needs no manifest to be
/// evaluated, which matters because under
/// [`ProjectExecutionTrust::Untrusted`] nothing may be loaded at all.
///
/// # Errors
/// When the index cannot be read.
pub fn discover_packages(
    connection: &Connection,
    trust: ProjectExecutionTrust,
) -> Result<RustProjectConfig, LifecycleError> {
    discover_packages_under(connection, trust, None)
}

/// The same, reading manifests under `workspace_root` so their declared
/// package names can be recorded.
///
/// # Errors
/// When the index cannot be read.
pub fn discover_packages_under(
    connection: &Connection,
    trust: ProjectExecutionTrust,
    workspace_root: Option<&Path>,
) -> Result<RustProjectConfig, LifecycleError> {
    let mut config = RustProjectConfig {
        trust,
        ..RustProjectConfig::default()
    };
    for resource in active_resources(connection)? {
        let name = file_name(&resource.path_key);
        if name == MANIFEST {
            config.manifests.push(resource);
        } else if name == LOCKFILE {
            config.lockfile = Some(resource);
        } else if ENVIRONMENT_NAMES.contains(&name)
            && (name != "config.toml" || resource.path_key.contains(".cargo/"))
        {
            config.environment.push(resource);
        }
    }

    let package_dirs: Vec<(String, String)> = config
        .manifests
        .iter()
        .map(|resource| (directory_of(&resource.path_key), resource.path_key.clone()))
        .collect();
    for (resource, path_key) in rust_sources(connection)? {
        if let Some(package) = nearest_package(&package_dirs, &path_key) {
            config.owning_package.insert(resource, package);
        }
    }

    if let Some(root) = workspace_root {
        for manifest in &config.manifests {
            let Ok(text) = fs::read_to_string(root.join(&manifest.path_rel)) else {
                continue;
            };
            if let Some(name) = declared_package_name(&text) {
                config.package_names.insert(manifest.path_key.clone(), name);
            }
        }
    }
    Ok(config)
}

/// The `name` a manifest's `[package]` section declares.
///
/// Read as text and not evaluated. A workspace root that declares no
/// package yields nothing, which is the truth about it rather than a
/// guess — and nothing here reimplements Cargo's manifest semantics,
/// because the backend already has them.
#[must_use]
pub fn declared_package_name(manifest_text: &str) -> Option<String> {
    let mut in_package = false;
    for line in manifest_text.lines() {
        let line = line.trim();
        if line.starts_with('[') {
            in_package = line == "[package]";
            continue;
        }
        if !in_package {
            continue;
        }
        if let Some(value) = line.strip_prefix("name") {
            let value = value.trim_start().strip_prefix('=')?.trim();
            return Some(value.trim_matches('"').to_owned());
        }
    }
    None
}

fn nearest_package(package_dirs: &[(String, String)], path_key: &str) -> Option<String> {
    package_dirs
        .iter()
        .filter(|(directory, _)| under_directory(path_key, directory))
        .max_by_key(|(directory, _)| directory.len())
        .map(|(_, manifest)| manifest.clone())
}

fn under_directory(path_key: &str, directory: &str) -> bool {
    directory.is_empty() || path_key.starts_with(&format!("{directory}/"))
}

fn directory_of(path_key: &str) -> String {
    path_key
        .rsplit_once('/')
        .map_or_else(String::new, |(directory, _)| directory.to_owned())
}

fn file_name(path_key: &str) -> &str {
    path_key.rsplit('/').next().unwrap_or(path_key)
}

fn content_identity(resource: &Resource) -> String {
    resource
        .content_hash
        .clone()
        .unwrap_or_else(|| resource.fingerprint.clone())
}

// ---------------------------------------------------------------------
// Environment identity
// ---------------------------------------------------------------------

/// How much of the resolution environment could actually be proven.
#[derive(Debug, Clone, PartialEq, Eq)]
pub enum EnvironmentAssurance {
    /// The toolchain is pinned and the dependency graph is locked.
    Proven,
    /// Something can change without the fingerprint moving. Still
    /// deterministic; just not evidence.
    Unknown { reason: String },
}

impl EnvironmentAssurance {
    #[must_use]
    pub const fn is_proven(&self) -> bool {
        matches!(self, Self::Proven)
    }
}

/// The environment a Rust semantic result depends on.
#[derive(Debug, Clone, PartialEq, Eq)]
pub struct EnvironmentIdentity {
    pub fingerprint: String,
    pub assurance: EnvironmentAssurance,
    /// Whether `Cargo.lock` pins the dependency graph.
    pub locked: bool,
    /// Whether the standard library's source is available.
    ///
    /// Its absence narrows navigation *into* std and nothing else.
    pub rust_src: bool,
}

/// Fingerprint the toolchain and what the compilation resolves against.
///
/// The backend build, the compiler, the host triple, the sysroot's
/// *name*, the trust mode, the safe-configuration switches, and every
/// manifest and lockfile by content identity.
///
/// What is deliberately absent: `~/.cargo/registry`, `~/.cargo/git`,
/// `~/.rustup` and `target/`. Hashing a package cache would make the
/// fingerprint a property of the machine rather than of the project,
/// and it is the lockfile that says what was resolved.
///
/// # Errors
/// Never; the signature matches every other backend's so a caller can
/// treat them uniformly.
pub fn environment_identity(
    install: &super::launcher::RustInstall,
    config: &RustProjectConfig,
) -> Result<EnvironmentIdentity, LifecycleError> {
    let mut fields: Vec<(String, String)> = vec![
        ("language_server".to_owned(), install.server_version.clone()),
        ("rustc".to_owned(), install.rustc_version.clone()),
        ("host_triple".to_owned(), install.host_triple.clone()),
        ("sysroot".to_owned(), install.sysroot_identity.clone()),
        ("project_trust".to_owned(), config.trust.as_str().to_owned()),
        (
            "backend_configuration".to_owned(),
            super::protocol::SAFE_CONFIGURATION.to_owned(),
        ),
        ("features".to_owned(), config.features.join(",")),
        (
            "target".to_owned(),
            config.target.clone().unwrap_or_default(),
        ),
    ];
    let mut unproven: Option<String> = None;

    for resource in config
        .manifests
        .iter()
        .chain(config.lockfile.iter())
        .chain(&config.environment)
    {
        fields.push((
            format!("cargo:{}", resource.path_key),
            content_identity(resource),
        ));
    }

    let locked = config.lockfile.is_some();
    if !locked {
        unproven = Some(format!("no {LOCKFILE} pins the resolved dependency graph"));
    }
    if config.manifests.is_empty() && unproven.is_none() {
        unproven = Some(format!("no {MANIFEST} defines a package"));
    }
    if install.rustc_version.is_empty() && unproven.is_none() {
        unproven = Some("the compiler this analysis runs against was not recorded".to_owned());
    }

    let borrowed: Vec<(&str, &str)> = fields
        .iter()
        .map(|(name, value)| (name.as_str(), value.as_str()))
        .collect();
    Ok(EnvironmentIdentity {
        fingerprint: db::fingerprint("rust-semantic-env-1", &borrowed),
        assurance: match unproven {
            None => EnvironmentAssurance::Proven,
            Some(reason) => EnvironmentAssurance::Unknown { reason },
        },
        locked,
        rust_src: install.rust_src,
    })
}

// ---------------------------------------------------------------------
// Module inventory
// ---------------------------------------------------------------------

/// A deterministic identity for the compilation's source set.
///
/// Every `.rs` and every manifest. A new file changes it even before a
/// `mod` declaration makes it a module, which is correct: the inventory
/// says what is *there*, and the backend decides what it means.
///
/// # Errors
/// When the index cannot be read.
pub fn inventory_fingerprint(connection: &Connection) -> Result<String, rusqlite::Error> {
    let mut statement = connection.prepare(
        "SELECT path_key FROM resource \
         WHERE state = 'ACTIVE' AND (language = 'RUST' \
               OR path_key LIKE '%Cargo.toml' OR path_key LIKE '%Cargo.lock') \
         ORDER BY path_key",
    )?;
    let keys: Vec<String> = statement
        .query_map([], |row| row.get(0))?
        .collect::<Result<_, _>>()?;
    let fields: Vec<(&str, &str)> = keys.iter().map(|key| ("module", key.as_str())).collect();
    Ok(db::fingerprint("rust-semantic-inventory-1", &fields))
}

// ---------------------------------------------------------------------
// Change sets
// ---------------------------------------------------------------------

/// What happened to one Resource.
#[derive(Debug, Clone, Copy, PartialEq, Eq, PartialOrd, Ord)]
pub enum ChangeKind {
    Added,
    Changed,
    Deleted,
    Moved,
}

impl ChangeKind {
    const fn watched(self) -> WatchedChangeKind {
        match self {
            Self::Added => WatchedChangeKind::Created,
            Self::Changed | Self::Moved => WatchedChangeKind::Changed,
            Self::Deleted => WatchedChangeKind::Deleted,
        }
    }

    const fn moves_inventory(self) -> bool {
        matches!(self, Self::Added | Self::Deleted | Self::Moved)
    }
}

/// Which synchronization path a change needs.
#[derive(Debug, Clone, Copy, PartialEq, Eq, PartialOrd, Ord)]
pub enum ChangeClass {
    /// Source. Hand the backend the current bytes; no reload.
    ///
    /// This covers a module tree change too, because in Rust the module
    /// tree *is* source: `mod runner;` is a line in a file, not an
    /// entry in a manifest.
    DocumentContent,
    /// The project's own definition — a manifest, the lockfile, the
    /// Cargo config, the toolchain file. Reload and wait.
    ProjectDefinition,
}

/// One Workspace change, as the lifecycle sees it.
#[derive(Debug, Clone, PartialEq, Eq)]
pub struct ResourceChange {
    pub resource: ResourceId,
    pub kind: ChangeKind,
    pub path_rel: String,
    pub previous_path_rel: Option<String>,
    pub language: Option<ResourceLanguage>,
}

impl ResourceChange {
    /// A change to a Rust source file. Use [`Self::with_language`] for
    /// a manifest or a lockfile.
    #[must_use]
    pub fn new(resource: ResourceId, kind: ChangeKind, path_rel: impl Into<String>) -> Self {
        Self {
            resource,
            kind,
            path_rel: path_rel.into(),
            previous_path_rel: None,
            language: Some(ResourceLanguage::Rust),
        }
    }

    #[must_use]
    pub fn from_previous(mut self, path_rel: impl Into<String>) -> Self {
        self.previous_path_rel = Some(path_rel.into());
        self
    }

    #[must_use]
    pub const fn with_language(mut self, language: Option<ResourceLanguage>) -> Self {
        self.language = language;
        self
    }

    /// Which synchronization path this change needs.
    ///
    /// Adding a `.rs` is *not* a project change, and that is the whole
    /// difference from a manifest-driven language: the file is not a
    /// module until some source says `mod`, and saying it is an edit
    /// the document sync already carries.
    #[must_use]
    pub fn class(&self) -> ChangeClass {
        if touches(self, is_project_definition) {
            ChangeClass::ProjectDefinition
        } else {
            ChangeClass::DocumentContent
        }
    }

    /// Whether this Resource is part of the compilation's source set.
    #[must_use]
    pub fn is_compilation_source(&self) -> bool {
        self.language == Some(ResourceLanguage::Rust)
    }
}

fn touches(change: &ResourceChange, predicate: impl Fn(&str) -> bool) -> bool {
    predicate(&change.path_rel) || change.previous_path_rel.as_deref().is_some_and(&predicate)
}

fn is_project_definition(path_rel: &str) -> bool {
    let name = file_name(path_rel);
    name == MANIFEST
        || name == LOCKFILE
        || name == "rust-toolchain.toml"
        || name == "rust-toolchain"
        || (name == "config.toml" && path_rel.contains(".cargo/"))
}

fn is_environment_candidate(path_rel: &str) -> bool {
    let name = file_name(path_rel);
    name == LOCKFILE || ENVIRONMENT_NAMES.contains(&name)
}

/// What one batch of Workspace changes means for one context.
#[derive(Debug, Clone, Default, PartialEq, Eq)]
pub struct ChangePlan {
    pub affected: BTreeSet<SemanticOwner>,
    pub inventory_moved: bool,
    pub config_moved: bool,
    pub environment_moved: bool,
    /// Whether any change in the batch needs a project reload.
    pub needs_reload: bool,
}

impl ChangePlan {
    #[must_use]
    pub fn is_empty(&self) -> bool {
        self.affected.is_empty()
            && !self.inventory_moved
            && !self.config_moved
            && !self.environment_moved
    }
}

/// Which owner contributions a change batch invalidates.
///
/// # Errors
/// When the index cannot be read.
pub fn plan_changes(
    index: &SemanticIndex,
    context: &AnalysisContext,
    changes: &[ResourceChange],
    config: &RustProjectConfig,
) -> Result<ChangePlan, LifecycleError> {
    let context_key = context.context_key();
    let mut plan = ChangePlan::default();
    let project_files: BTreeSet<ResourceId> = config
        .manifests
        .iter()
        .chain(config.lockfile.iter())
        .chain(&config.environment)
        .map(|resource| resource.id)
        .collect();

    for change in changes {
        for owner in index.owners_depending_on(change.resource)? {
            if owner.context_key == context_key {
                plan.affected.insert(owner);
            }
        }
        if change.class() == ChangeClass::ProjectDefinition {
            plan.needs_reload = true;
        }
        if change.is_compilation_source() && change.kind.moves_inventory() {
            plan.inventory_moved = true;
        }
        if project_files.contains(&change.resource) || touches(change, is_project_definition) {
            plan.config_moved = true;
        }
        if touches(change, is_environment_candidate) {
            plan.environment_moved = true;
        }
    }

    if plan.inventory_moved || plan.config_moved || plan.environment_moved {
        plan.affected.extend(index.owners_of_context(&context_key)?);
        return Ok(plan);
    }

    let touched: Vec<ResourceId> = changes.iter().map(|change| change.resource).collect();
    let published: BTreeSet<SemanticOwner> =
        index.owners_of_context(&context_key)?.into_iter().collect();
    for dependent in graph_lifecycle::dependents_of(index.connection(), &touched, false)? {
        let owner = SemanticOwner::new(&context_key, dependent);
        if published.contains(&owner) {
            plan.affected.insert(owner);
        }
    }
    Ok(plan)
}

// ---------------------------------------------------------------------
// Synchronization
// ---------------------------------------------------------------------

/// The backend notification for one change batch.
#[must_use]
pub fn watched_changes(workspace_root: &Path, changes: &[ResourceChange]) -> Vec<WatchedChange> {
    let mut by_uri: BTreeMap<String, WatchedChangeKind> = BTreeMap::new();
    for change in changes {
        if let Some(previous) = &change.previous_path_rel
            && previous != &change.path_rel
        {
            by_uri.insert(
                path_to_uri(&workspace_root.join(previous)),
                WatchedChangeKind::Deleted,
            );
        }
        by_uri.insert(
            path_to_uri(&workspace_root.join(&change.path_rel)),
            change.kind.watched(),
        );
    }
    by_uri
        .into_iter()
        .map(|(uri, kind)| WatchedChange { uri, kind })
        .collect()
}

/// Who hands out the monotonic document versions and remembers what was
/// sent.
///
/// The text and not merely the name, because a re-synchronization is a
/// `didChange` that says what it replaces — the server declares
/// incremental sync. A restarted connection has none of this, which is
/// the point: it has opened nothing.
pub trait DocumentVersions {
    fn next_document_version(&self) -> i64;

    /// The text this connection last handed the backend for `uri`, and
    /// records `text` as what it holds now. `None` means new.
    fn exchange_text(&self, uri: &str, text: &str) -> Option<String>;
}

/// Who can wait for the backend to settle.
pub trait QuiescenceBarrier {
    /// How many times this connection has seen the server settle.
    fn settlings_seen(&self) -> u64;
    /// Block until the count moves past `seen`, or time out.
    ///
    /// # Errors
    /// When the server did not settle.
    fn wait_for_quiescent(&self, seen: u64) -> Result<usize, String>;
}

/// Hand the backend Brainprint's current bytes for every changed
/// source file.
///
/// What is sent is what the Workspace indexed and what is on disk.
/// Nothing unsaved is ever accepted, because there is nothing unsaved
/// to accept.
///
/// A document is opened once; a later change carries the range it
/// replaces.
///
/// # Errors
/// When a notification could not be delivered.
pub fn synchronize_documents(
    queries: &dyn adapter::RustQueries,
    documents: &dyn DocumentVersions,
    workspace_root: &Path,
    changes: &[ResourceChange],
) -> Result<usize, LifecycleError> {
    let mut sent = 0;
    for change in changes {
        if change.kind == ChangeKind::Deleted || !change.is_compilation_source() {
            continue;
        }
        let path = workspace_root.join(&change.path_rel);
        let Ok(text) = fs::read_to_string(&path) else {
            continue;
        };
        let uri = path_to_uri(&path);
        let version = documents.next_document_version();
        let previous = documents.exchange_text(&uri, &text);
        adapter::synchronize_document(queries, uri, text, version, previous.as_deref())
            .map_err(|failure| LifecycleError::Sync(failure.to_string()))?;
        sent += 1;
    }
    Ok(sent)
}

/// Announce a project-definition change and wait until the backend has
/// re-read it.
///
/// Three steps, in this order: the filesystem notification says what
/// moved, [`method::RELOAD_WORKSPACE`] makes the server read the
/// manifests again, and the barrier waits for it to settle.
///
/// Refused outright when the Workspace is untrusted: re-reading the
/// manifests is reading what the Workspace wrote, and nothing here
/// infers permission. The caller gets [`LifecycleError::Untrusted`] and
/// can degrade, which is the honest outcome.
///
/// # Errors
/// When the Workspace is untrusted, a notification fails, or the
/// backend does not settle.
pub fn reload_projects(
    queries: &dyn adapter::RustQueries,
    barrier: &dyn QuiescenceBarrier,
    workspace_root: &Path,
    changes: &[ResourceChange],
    config: &RustProjectConfig,
) -> Result<usize, LifecycleError> {
    if !config.trust.may_load_projects() {
        return Err(LifecycleError::Untrusted(
            method::RELOAD_WORKSPACE.to_owned(),
        ));
    }
    let batch = watched_changes(workspace_root, changes);
    if !batch.is_empty() {
        adapter::notify_watched_files(queries, batch)
            .map_err(|failure| LifecycleError::Sync(failure.to_string()))?;
    }
    let seen = barrier.settlings_seen();
    adapter::reload_workspace(queries)
        .map_err(|failure| LifecycleError::Sync(failure.to_string()))?;
    barrier
        .wait_for_quiescent(seen)
        .map_err(LifecycleError::Sync)
}

// ---------------------------------------------------------------------
// Lifecycle steps
// ---------------------------------------------------------------------

/// Why a lifecycle step could not complete.
#[derive(Debug)]
pub enum LifecycleError {
    Index(SemanticIndexError),
    Structural(Box<crate::scan::ScanError>),
    Merge(merge::MergeError),
    Sqlite(rusqlite::Error),
    Refresh(Box<RustSemanticError>),
    /// A notification or barrier could not complete, so nothing sent
    /// after it may be treated as current.
    Sync(String),
    /// The step would have read the Workspace's own project definition
    /// in a Workspace that has not been trusted for it.
    Untrusted(String),
}

impl fmt::Display for LifecycleError {
    fn fmt(&self, formatter: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            Self::Index(error) => write!(formatter, "semantic index: {error}"),
            Self::Structural(error) => write!(formatter, "structural plan: {error}"),
            Self::Merge(error) => write!(formatter, "semantic merge: {error}"),
            Self::Sqlite(error) => write!(formatter, "index: {error}"),
            Self::Refresh(error) => write!(formatter, "rust refresh: {error}"),
            Self::Sync(detail) => write!(formatter, "backend synchronization: {detail}"),
            Self::Untrusted(method) => write!(
                formatter,
                "{method} re-reads this Workspace's own manifests, and it is not trusted for that"
            ),
        }
    }
}

impl Error for LifecycleError {}

impl From<SemanticIndexError> for LifecycleError {
    fn from(error: SemanticIndexError) -> Self {
        Self::Index(error)
    }
}
impl From<crate::scan::ScanError> for LifecycleError {
    fn from(error: crate::scan::ScanError) -> Self {
        Self::Structural(Box::new(error))
    }
}
impl From<merge::MergeError> for LifecycleError {
    fn from(error: merge::MergeError) -> Self {
        Self::Merge(error)
    }
}
impl From<rusqlite::Error> for LifecycleError {
    fn from(error: rusqlite::Error) -> Self {
        Self::Sqlite(error)
    }
}
impl From<RustSemanticError> for LifecycleError {
    fn from(error: RustSemanticError) -> Self {
        Self::Refresh(Box::new(error))
    }
}

/// What one withdrawal pass removed.
#[derive(Debug, Clone, Default, PartialEq, Eq)]
pub struct WithdrawReport {
    pub owners: Vec<SemanticOwner>,
    pub gaps_restored: usize,
    pub relations_removed: usize,
}

/// Withdraw every affected owner's contribution, then mark it dirty.
///
/// Runs *before* structural replacement, for the reason
/// `semantic_evidence.relation_id` has no cascade.
///
/// # Errors
/// When the index cannot be written.
pub fn withdraw_affected(
    index: &SemanticIndex,
    owners: &BTreeSet<SemanticOwner>,
    error_code: &str,
) -> Result<WithdrawReport, LifecycleError> {
    let connection = index.connection();
    let mut report = WithdrawReport::default();
    for owner in owners {
        let transaction = connection.unchecked_transaction()?;
        let outcome = merge::withdraw(&transaction, &owner.context_key, Some(owner.owner))?;
        transaction.commit()?;
        report.gaps_restored += outcome.gaps_restored;
        report.relations_removed += outcome.relations_removed;
        index.mark_dirty(owner, error_code)?;
        report.owners.push(owner.clone());
    }
    Ok(report)
}

/// Mark every affected owner unavailable, keeping its last valid
/// publication readable.
///
/// # Errors
/// When the index cannot be written.
pub fn mark_unavailable(
    index: &SemanticIndex,
    owners: &BTreeSet<SemanticOwner>,
    error_code: &str,
) -> Result<(), LifecycleError> {
    for owner in owners {
        index.mark_unavailable(owner, error_code)?;
    }
    Ok(())
}

/// Re-check every owner published for one context against the inputs as
/// they are now.
///
/// # Errors
/// When the index cannot be read.
pub fn revalidate_context(
    index: &SemanticIndex,
    context: &AnalysisContext,
    current: &CurrentInputs,
) -> Result<Vec<(SemanticOwner, SemanticStatus)>, LifecycleError> {
    let mut answers = Vec::new();
    for owner in index.owners_of_context(&context.context_key())? {
        let status = index.revalidate(&owner, current)?;
        answers.push((owner, status));
    }
    Ok(answers)
}

/// The inputs one owner's publication is validated against.
#[must_use]
pub fn current_inputs(
    context: &AnalysisContext,
    config: &ConfigBasis,
    capabilities: &CapabilityReport,
    inventory: &str,
    environment: &EnvironmentIdentity,
) -> CurrentInputs {
    CurrentInputs::new(context, config, capabilities)
        .with_inventory(inventory)
        .with_environment_proven(environment.assurance.is_proven())
}

/// One owner's refresh outcome, or why it did not happen.
#[derive(Debug)]
pub enum OwnerOutcome {
    Refreshed(Box<RefreshOutcome>),
    Failed {
        owner: SemanticOwner,
        error: Box<RustSemanticError>,
    },
}

impl OwnerOutcome {
    #[must_use]
    pub const fn succeeded(&self) -> bool {
        matches!(self, Self::Refreshed(_))
    }
}

/// Refresh a set of owners, one at a time, on the shared runtime.
///
/// # Errors
/// When the index cannot be written.
pub fn refresh_owners(
    index: &SemanticIndex,
    queries: &dyn adapter::RustQueries,
    request: &mut RefreshRequest<'_>,
    owners: &BTreeSet<SemanticOwner>,
) -> Result<Vec<OwnerOutcome>, LifecycleError> {
    let mut outcomes = Vec::new();
    for owner in owners {
        request.owner = owner.owner;
        match super::refresh_resource(index, queries, request) {
            Ok(outcome) => outcomes.push(OwnerOutcome::Refreshed(Box::new(outcome))),
            Err(error) => {
                index.mark_dirty(owner, BACKEND_UNAVAILABLE_CODE)?;
                outcomes.push(OwnerOutcome::Failed {
                    owner: owner.clone(),
                    error: Box::new(error),
                });
            }
        }
    }
    Ok(outcomes)
}

/// Mark every owner in a context not current because the configuration
/// that governs it changed.
///
/// # Errors
/// When the index cannot be written.
pub fn invalidate_for_config(
    index: &SemanticIndex,
    context: &AnalysisContext,
) -> Result<BTreeSet<SemanticOwner>, LifecycleError> {
    let owners: BTreeSet<SemanticOwner> = index
        .owners_of_context(&context.context_key())?
        .into_iter()
        .collect();
    for owner in &owners {
        index.mark_dirty(owner, CONFIG_CHANGED_CODE)?;
    }
    Ok(owners)
}

/// What a whole merge pass did, summed.
#[must_use]
pub fn total_merged(outcomes: &[OwnerOutcome]) -> MergeOutcome {
    let mut total = MergeOutcome::default();
    for outcome in outcomes {
        if let OwnerOutcome::Refreshed(refreshed) = outcome {
            total.gaps_resolved += refreshed.merged.gaps_resolved;
            total.gaps_restored += refreshed.merged.gaps_restored;
            total.corroborated += refreshed.merged.corroborated;
            total.relations_created += refreshed.merged.relations_created;
            total.relations_removed += refreshed.merged.relations_removed;
            total.conflicts += refreshed.merged.conflicts;
        }
    }
    total
}

// ---------------------------------------------------------------------
// Index reads
// ---------------------------------------------------------------------

type ResourceRow = (Vec<u8>, String, String, String, Option<String>);

fn row_to_resource(row: ResourceRow) -> Option<Resource> {
    let (uid, path_rel, path_key, revision, content_hash) = row;
    let bytes: [u8; 16] = uid.as_slice().try_into().ok()?;
    Some(Resource {
        id: ResourceId::from_bytes(bytes),
        path_rel,
        path_key,
        kind: crate::resource::ResourceKind::File,
        role: crate::resource::ResourceRole::Config,
        language: None,
        size_bytes: 0,
        mtime_ns: 0,
        fingerprint: revision.clone(),
        content_hash,
        state: crate::resource::ResourceState::Active,
        resource_revision: revision,
        generated_kind: None,
        container_resource_id: None,
    })
}

fn active_resources(connection: &Connection) -> Result<Vec<Resource>, LifecycleError> {
    let mut statement = connection.prepare(
        "SELECT uid, path_rel, path_key, resource_revision, content_hash \
         FROM resource WHERE state = 'ACTIVE' ORDER BY path_key",
    )?;
    let rows: Vec<ResourceRow> = statement
        .query_map([], |row| {
            Ok((
                row.get(0)?,
                row.get(1)?,
                row.get(2)?,
                row.get(3)?,
                row.get(4)?,
            ))
        })?
        .collect::<Result<_, _>>()?;
    Ok(rows.into_iter().filter_map(row_to_resource).collect())
}

fn rust_sources(connection: &Connection) -> Result<Vec<(ResourceId, String)>, LifecycleError> {
    let mut statement = connection.prepare(
        "SELECT uid, path_key FROM resource \
         WHERE state = 'ACTIVE' AND language = 'RUST' ORDER BY path_key",
    )?;
    let rows: Vec<(Vec<u8>, String)> = statement
        .query_map([], |row| Ok((row.get(0)?, row.get(1)?)))?
        .collect::<Result<_, _>>()?;
    Ok(rows
        .into_iter()
        .filter_map(|(uid, path_key)| {
            let bytes: [u8; 16] = uid.as_slice().try_into().ok()?;
            Some((ResourceId::from_bytes(bytes), path_key))
        })
        .collect())
}

/// One active Resource by path key.
///
/// # Errors
/// When the index cannot be read.
pub fn active_resource_by_path(
    connection: &Connection,
    path_key: &str,
) -> Result<Option<Resource>, LifecycleError> {
    let row: Option<ResourceRow> = connection
        .query_row(
            "SELECT uid, path_rel, path_key, resource_revision, content_hash \
             FROM resource WHERE path_key = ?1 AND state = 'ACTIVE'",
            params![path_key],
            |row| {
                Ok((
                    row.get(0)?,
                    row.get(1)?,
                    row.get(2)?,
                    row.get(3)?,
                    row.get(4)?,
                ))
            },
        )
        .optional()?;
    Ok(row.and_then(row_to_resource))
}
