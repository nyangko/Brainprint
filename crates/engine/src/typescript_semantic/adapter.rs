...
/// The site vocabulary, shared with every other adapter that reads the
/// Occurrence table. Same index, same rules, one implementation.
pub use crate::semantic_normalize::{
    DeferredGap, ResolvableKind, RetainedExternal, STRUCTURALLY_EXTERNAL, SemanticSite, SiteSet,
    collect_sites, source_endpoint,
};
...
///
/// The site vocabulary is language-neutral; what a site *means* is not.
/// Which capability answered depends on where the target turned out to
/// be, and whether a call is a proven static target depends on the
/// language's own dispatch rules -- so both stay here.
trait TypeScriptSite {
...
impl TypeScriptSite for ResolvableKind {
